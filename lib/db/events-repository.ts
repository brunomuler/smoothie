...
export class EventsRepository {
  ...
  async getUserActions(
    ...
    const { limit = 50, offset = 0, actionTypes, poolId, assetAddress } = options
  ...
  async getBalanceHistoryFromEvents(
    ...
    // First, get the earliest event date for this user/asset
    ...
    const result = await pool.query(
      ...
      WITH date_range AS (
        SELECT generate_series(
          GREATEST(
            ...
            (SELECT MIN(DATE(ledger_closed_at)) FROM parsed_events WHERE user_address = $1 AND asset_address = $2)
          ),
          CURRENT_DATE,
          '1 day'::interval
        )::date AS date
      ),
      -- Get all events for this user/asset ordered by time
      user_events AS (
        SELECT
          pool_id,
      ...
      event_positions AS (
        SELECT
          ...
          SUM(CASE
            WHEN action_type IN ('supply') THEN amount_tokens / 10000000.0
            WHEN action_type IN ('withdraw') THEN -amount_tokens / 10000000.0
            ELSE 0
          END) OVER (PARTITION BY pool_id ORDER BY ledger_closed_at, ledger_sequence) AS supply_btokens,
          -- Cumulative collateral tokens (supply_collateral/withdraw_collateral) - convert from stroops
          SUM(CASE
            WHEN action_type IN ('supply_collateral') THEN amount_tokens / 10000000.0
            WHEN action_type IN ('withdraw_collateral') THEN -amount_tokens / 10000000.0
            ELSE 0
          END) OVER (PARTITION BY pool_id ORDER BY ledger_closed_at, ledger_sequence) AS collateral_btokens,
          -- Cumulative debt tokens (borrow/repay) - convert from stroops
          SUM(CASE
            WHEN action_type IN ('borrow') THEN amount_tokens / 10000000.0
            WHEN action_type IN ('repay') THEN -amount_tokens / 10000000.0
            ELSE 0
          ...
          SUM(CASE
            WHEN action_type IN ('borrow') THEN amount_underlying / 10000000.0
            ELSE 0
          ...
          SUM(CASE
            WHEN action_type IN ('repay') THEN amount_underlying / 10000000.0
            ELSE 0
