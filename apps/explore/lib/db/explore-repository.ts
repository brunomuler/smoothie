...
export class ExploreRepository {
  ...
  async getAccountsByBalance(params: {
    ...
    const { assetAddress, minBalance, inUsd, limit, offset, orderDir = 'desc', hasBorrows, hasDeposits } = params
    ...
    // Build position filter conditions
    ...
    const balanceQuery = `
      ...
      FROM user_balances
      WHERE (supply_balance + collateral_balance - debt_balance) >= $2${positionFilterClause}
  ...
  /**
   * Get pool statistics - event count and unique event types per pool
   */
  async getPoolStatistics(params: {
    ...
    // Main query - get pool statistics with event counts
    const result = await pool.query(
      `
      SELECT
        p.pool_id,
